...
@app.route('/get_nearby_hospitals', methods=['GET']) # Get all nearby hospitals
def get_nearby_hospitals():
    data = request.headers['data']
    if data==None:
        return "", 400 # Incorrect address
    else:
        radius="10000" # Get hospitals within radius 10000
        textdata = []
        data = data.strip("\"")
        url = config.geocodeurl+data+"&key="+config.apikey # Geocode url Google api
        response = requests.request("GET", url, headers=headers, data=payload)
        res=json.loads(response.text)
        for j in res["results"]: # Iterate over the response results
            geo = j["geometry"]
            lat = j["geometry"]["location"]["lat"]
            lng = j["geometry"]["location"]["lng"]
            place = j["place_id"]
            url = config.searchurl+str(lat)+"%2C"+str(lng)+"&radius="+radius+"&type=hospital&key="+config.apikey #Search url google api
            response = requests.request("GET", url, headers=headers, data=payload)
            res = json.loads(response.text)
            n = len(res["results"])
            for _ in res["results"]:
                pid = _["place_id"]
                url = config.placeurl+pid+"&key="+config.apikey #places url google api
                response = requests.request("GET", url, headers=headers, data=payload)
                resp = json.loads(response.text)
                resp = resp["result"] 
                if 'name' not in resp: #Check formats
                    name = None
                else:
                    name = resp["name"]
                ...
                if 'rating' not in resp:
                    ...
                    rating = resp["rating"]
                url = config.distanceurl+place+"&destinations=place_id:"+pid+"&key="+config.apikey #Distance matrix url google api
                response = requests.request("GET", url, headers=headers, data=payload)
                newres = json.loads(response.text)
                dist = newres["rows"][0]["elements"][0]["distance"]["text"]
                dur = newres["rows"][0]["elements"][0]["duration"]["text"]
                space = "@#thisisspace$%"
                textdata.append(name+space+str(lat)+space+str(lng)+space+address+space+dist+space+dur+space+pid+space+place) #We have the results
                result = database_helper.hospital_exist(pid) #Check existance of hospital in database
                if not result:
                    res = database_helper.store_hospdata(pid, name, address, pluscode, lat, lng, phone, website, rating) #Add each hospital to database
                    if res:
...
@app.route('/get_hospital_details', methods=['GET']) # Get details of specific hospital
def get_hospital_details():
    data = request.headers['data']
    if data==None:
        return "", 400 # No hospital id
    else:
        data = data.strip("\"")
        data = database_helper.get_hospdata(data) # Fetch details of hospital with place_id=data from database
        return jsonify({'data': data}), 200

@app.route('/get_all_hospitals', methods=['GET']) # Get all hospitals' info
def get_all_hospitals():
    data = database_helper.get_allhospdata() # Fetch from database
    return jsonify({'data': data}), 200

@app.route('/get_all_departments', methods=['GET']) # Get all departments' info
def get_all_departments():
    data = database_helper.get_alldeptdata() # Fetch from database
    return jsonify({'data': data}), 200

@app.route('/get_all_doctors', methods=['GET']) # Get all doctors' information
def get_all_doctors():
    data = database_helper.get_alldoctdata() # Fetch from database
    return jsonify({'data': data}), 200
    ...
    if 'hospital' not in data or 'department' not in data or 'doctor' not in data or 'name' not in data or 'email' not in data or 'datetime' not in data or 'token' not in data:
        ...
        if hospital == "" or department == "" or doctor == "" or name == "" or email == "" or datetime == "" or token == "":
            ...
            if check!=None:
                ...
                msg.body = 'Hello '+name+', this mail is to confirm your appointment on Care Connect, at '+hospital+' in '+department+' with '+doctor+' on '+date+' at time '+time+' . Kindly reach on time.'; 
                mail.send(msg) 
                return "", 200
            return "", 401
        return "", 400
